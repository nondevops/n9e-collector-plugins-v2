...
# ping 172.26.45.178 -c 10 | grep -v "grep" | grep -v "172.26.45.178" > ping_ip.tmp
ping $ip -c $count | grep -v "grep" | grep -v "$ip" > $tmp

# cat ping_ip.tmp | awk -F "," 'NR==2{print $1}' | awk -F " " '{print $1}'
#packets_transmitted=$(cat $tmp | awk -F "," 'NR==2{print $1}' | awk -F " " '{print $1}')

# cat ping_ip.tmp | awk -F "," 'NR==2{print $2}' | awk -F " " '{print $1}'
#received=$(cat $tmp | awk -F "," 'NR==2{print $2}' | awk -F " " '{print $1}')

# cat ping_ip.tmp | awk -F "," 'NR==2{print $3}' | awk -F " " '{print $1}' | awk -F "%" '{print $1}'
packet_loss=$(cat $tmp | awk -F "," 'NR==2{print $3}' | awk -F " " '{print $1}' | awk -F "%" '{print $1}')

# cat ping_ip.tmp | grep -v "grep" | grep "rtt" | awk '{print $4}' | awk -F '/' '{print $4}'
